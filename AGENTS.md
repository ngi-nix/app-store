...
Other packages built by NGI Forge can be referenced in recipes using `configRoot.packages`:

```nix
{
  # Reference another NGI Forge package
  packages.run = [
    configRoot.packages.gdal  # Access gdal from NGI Forge
  ];
}
```
...
services = {
  my-service = {
    command = configRoot.packages.my-package;  # Package or string
...
programs = {
  packages = [
    configRoot.packages.my-package  # Reference packages from forge
    pkgs.curl
    pkgs.jq
  ];
...
container = {
  ...
  packages = [
    configRoot.packages.my-package  # Packages to include in /bin
  ];
...

{
  ...
  services.python-web = {
    command = configRoot.packages.python-web;
    argv = [ "--host" "0.0.0.0" ];
    environment = {
  ...
  programs = {
    packages = [
      configRoot.packages.python-web
      pkgs.curl
      pkgs.postgresql
    ];
  ...
  container = {
    enable = true;
    tag = "latest";
    packages = [ configRoot.packages.python-web ];
    extraConfig = {
      Env = [ "PORT=5000" ];
