...
scopeRecipeOptions : NixPath -> Trees NodeNixOption -> Trees NodeNixOption
...
filterRecipeOptions : RouteRecipeOptions -> NixPath -> Trees NodeNixOption -> Trees NodeNixOptionFiltered
        ...
        |> List.map
            (\tree ->
                case tree |> Tree.label of
                    ( n, vs ) ->
                        Tree.tree
                            (if vs |> List.any (filterRecipeOption route path) then
...
filterRecipeOption route path option =
    let
        searchPattern =
        ...
        option_name =
            String.toLower (joinNixPath path)
