    ...
    installPhase = ''
      ...
      export PATH=$\PATH:${
        lib.makeBinPath [
          pkgs.coreutils
          pkgs.sqlite
          configRoot.packages.ironcalc-server
        ]
      }
      ...
      if [ ! -f "\$IRONCALC_DB_PATH" ]; then
        ...
        sqlite3 "\$IRONCALC_DB_PATH" < "${configRoot.packages.ironcalc-server}/share/ironcalc/init_db.sql"
      fi

      export ROCKET_DATABASES="{ironcalc={url=\"\$IRONCALC_DB_PATH\"}}"
      export IRONCALC_WEBAPP_DIR="\''${IRONCALC_WEBAPP_DIR:-${configRoot.packages.ironcalc-frontend}}"
      exec ironcalc_server "\$@"
      EOF
      chmod +x $out/bin/ironcalc

      ln -s ${configRoot.packages.ironcalc-tools}/bin/xlsx_2_icalc $out/bin/xlsx_2_icalc
    '';
  };
}
