...

{
  ...
  services = {
    components.mox = {
      ...
      '';
      command = configRoot.packages.mox;
      argv = [
        "-config"
    ...
    runtimes = {
      container = {
        ...
        setup = ''
          ...
          if ! [ -d /var/lib/mox ]; then
            ...
            # Generate DKIM keys
            mkdir -p config/dkim
            ${configRoot.packages.mox}/bin/mox dkim genrsa > config/dkim/dkima.rsa2048.privatekey.pkcs8.pem
            ${configRoot.packages.mox}/bin/mox dkim genrsa > config/dkim/dkimb.rsa2048.privatekey.pkcs8.pem
        ...
        packages = [
          ...
          configRoot.packages.mox # required for admin tasks
      ...
      nixos = {
        ...
        '';
        packages = [ configRoot.packages.mox ];
        extraConfig = {
